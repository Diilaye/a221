...
class _CategorieScreenState extends State<CategorieScreen> {
  ...
  List<dynamic> _getFilteredCategories(CategorieBloc categorieBloc) {
    List<dynamic> filtered = categorieBloc.categories;

    // Appliquer le filtre de recherche
    if (categorieBloc.recherche.isNotEmpty) {
      filtered = filtered
          .where((cat) => cat.titre!
              .toLowerCase()
              .contains(categorieBloc.recherche.toLowerCase()))
          .toList();
    }

    // Appliquer le filtre de statut
    if (statusFilter == 'En ligne') {
      filtered = filtered.where((cat) => cat.statusOnline == "on").toList();
  ...
  List<dynamic> _getPaginatedCategories(CategorieBloc categorieBloc) {
    final filtered = _getFilteredCategories(categorieBloc);
  ...
  int _getTotalPages(CategorieBloc categorieBloc) {
    final filtered = _getFilteredCategories(categorieBloc);
      ...
      children: [
        Container(
          ...
          child: ListView(
            ...
            children: [
              _buildHeader(context, categorieBloc, menuAdminBloc),
              const SizedBox(height: 32),
              _buildFiltersSection(context, categorieBloc),
              const SizedBox(height: 32),
              _buildCategoriesTable(context, categorieBloc),
              const SizedBox(height: 32),
              if (_getFilteredCategories(categorieBloc).isNotEmpty)
                _buildPagination(context, categorieBloc),
              const SizedBox(height: 32),
            ],
          ),
        ),
  ...
  Widget _buildHeader(BuildContext context, CategorieBloc categorieBloc, MenuAdminBloc menuAdminBloc) {
    return Container(
              ...
              Container(
                ...
                child: Row(
                  children: [
                    ...
                    const SizedBox(width: 6),
                    Text('Catégories', style: TextStyle(color: blanc, fontSize: 12, fontWeight: FontWeight.w500)),
                    const SizedBox(width: 6),
                    Icon(CupertinoIcons.chevron_forward, color: blanc.withOpacity(0.7), size: 10),
                    const SizedBox(width: 6),
                    ...
                    Row(
                      children: [
                        _buildStatCard(
                          ...
                          label: 'Total',
                          value: '${categorieBloc.categories.length}',
                        ),
                        const SizedBox(width: 16),
                        _buildStatCard(
                          icon: CupertinoIcons.checkmark_circle_fill,
                          label: 'En ligne',
                          value: '${categorieBloc.categories.where((e) => e.statusOnline == "on").length}',
                          color: vert,
                        ),
                        const SizedBox(width: 16),
                        _buildStatCard(
  ...
  Widget _buildFiltersSection(BuildContext context, CategorieBloc categorieBloc) {
    return Padding(
  ...
  Widget _buildCategoriesTable(BuildContext context, CategorieBloc categorieBloc) {
    final paginatedCategories = _getPaginatedCategories(categorieBloc);
        ...
        child: Column(
          children: [
            ...
            if (paginatedCategories.isEmpty)
              ...
              ...paginatedCategories.map((e) => Container(
                padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 16),
                decoration: BoxDecoration(
                      ...
                      child: Row(
                        children: [
                          MouseRegion(
                            ...
                            child: GestureDetector(
                              onTap: () {
                                categorieBloc.setShowUpdate(1);
                                categorieBloc.setCategorie(e);
                              },
                              child: Container(
  ...
  Widget _buildPagination(BuildContext context, CategorieBloc categorieBloc) {
    final filteredCategories = _getFilteredCategories(categorieBloc);
    final totalPages = _getTotalPages(categorieBloc);
    final startIndex = (currentPage - 1) * itemsPerPage + 1;
    final endIndex = (currentPage * itemsPerPage) > filteredCategories.length
    ...
    return Padding(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Text(
              "Affichage de $startIndex à $endIndex sur ${filteredCategories.length} catégories",
              style: TextStyle(
                color: noir.withOpacity(0.7),
                fontSize: 14,
                fontWeight: FontWeight.w600,
